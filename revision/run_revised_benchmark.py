# -*- coding: utf-8 -*-
"""
MoRO revision pipeline: MoRO-G + MoRO-Mix with retry + fallback accept
Existing experimental pipeline is preserved; revision functionality is added without changing default behavior.
...
def _euclid_dm_do_means(Xm: np.ndarray, Xo: np.ndarray, x: np.ndarray, k: int, epsilon: float = 1e-12) -> Tuple[float, float]:
    eps = float(epsilon)
...
def _mr_star_from_dm_do(dm_mean: float, do_mean: float, exponent: float = 2.0) -> float:
    # MR* = (do/dm)^q; default q=2 preserves the existing experiment.
    r = do_mean / dm_mean if dm_mean > 0 else 0.0
    return float(r ** float(exponent))
...
class MoROGSampler:
    ...
    def __init__(
        ...
        self.diag_: Optional[MoRODiag] = None
    ...
    def fit_resample(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(classes) < 2:
            self.diag_ = MoRODiag(np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
            return X, y
        ...
        for cls in classes:
            ...
            for _ in range(n_to_add):
                ...
                for att in range(self.max_attempts_per_sample):
                    ...
                    if is_borderline:
                        gate_active_cnt += 1
                        dm_mean, do_mean = _euclid_dm_do_means(Xm, Xo, x_cand, self.k_neighbors, epsilon=self.epsilon)
                        mr_star = _mr_star_from_dm_do(dm_mean, do_mean, exponent=self.mr_exponent)
...
class MoROMixSampler:
    """
    MoRO-Mix: same as v2 BUT retry+fallback, prob clipping.
    """
    def __init__(
        self,
        num_indices: List[int],
                ...
                for att in range(self.max_attempts_per_sample):
                    ...
                    if is_borderline:
                        ...
                        dm_mean, do_mean = _gower_dm_do_means(
                            ...
                            self.k_neighbors, epsilon=self.epsilon
                        )
                        mr_star = _mr_star_from_dm_do(dm_mean, do_mean, exponent=self.mr_exponent)
                        p = _sigmoid(self.alpha * (mr_star - 1.0))
                        p = float(np.clip(p, self.p_floor, self.p_ceil))
...
def get_samplers(detected_type: str, cat_indices: List[int], num_indices: List[int]) -> Dict[str, Any]:
    ...
    # =========
    # MoRO samplers; default parameters preserve the existing experiment
    # =========
    samplers["MoRO-G"] = MoROGSampler(
        k_neighbors=5,
        k_borderline=15,
        maj_ratio_thr=0.30,
        alpha=6.0,
    ...
    samplers["MoRO-G(no_gate)"] = MoROGSampler(
        k_neighbors=5,
        k_borderline=15,
        maj_ratio_thr=0.30,
        alpha=6.0,
    ...
    if detected_type == "mixed/nominal" and len(cat_indices) > 0:
        samplers["MoRO-Mix"] = MoROMixSampler(
        ...
        samplers["MoRO-Mix(no_gate)"] = MoROMixSampler(
            num_indices=num_indices,
            cat_indices=cat_indices,
            k_neighbors=5,
            k_borderline=15,
...
def generate_eswa_reports(raw_df: pd.DataFrame, summary_df: pd.DataFrame, primary_metric: str = "f1_macro", distribution_df: Optional[pd.DataFrame] = None) -> None:
    ...
    g2, ng2 = "MoRO-Mix", "MoRO-Mix(no_gate)"
...
# ======================================================================
# REVISION ADDITION: sensitivity runner
# ----------------------------------------------------------------------
# IMPORTANT: main() above is intentionally unchanged in its default
# numerical settings. Running this function is OPTIONAL and does not
# overwrite the original experiment outputs unless the user chooses to
# save into the same directory.
# ======================================================================
def run_sensitivity_experiments(output_prefix: str = "revision_sensitivity") -> pd.DataFrame:
    """Run compact, isolated MoRO sensitivity experiments.

    The baseline/default configuration is exactly the current experiment:
        k=5, k_borderline=15, tau=0.30, alpha=6, epsilon=1e-12, q=2.

    The runner evaluates alpha, tau_majority, epsilon and MR* exponent.
    It uses the same 20 datasets, preprocessing and 5-fold CV, but reports
    only MoRO variants. Existing main() outputs are not read or overwritten.

    NOTE: The experiment is intentionally separate from main() so that the
    previously generated results remain untouched.
    """
    sensitivity_grid = {
    ...
    for ds_name, ds_id in DATASETS_20:
        print(f"\n[SENS] Dataset: {ds_name} (OpenML ID={ds_id})")
        X_df, y_sr = load_openml_dataset(ds_id)
        num_cols, cat_cols = detect_feature_types(X_df)
        d_type = detected_type_label(num_cols, cat_cols)
        le = LabelEncoder()
        ...
        # Keep the sensitivity experiment compact: LR is sufficient for the
        # parameter-response analysis and avoids multiplying runtime by 3.
        model_name = "LR"
        model_template = models[model_name]

        for fold_idx, (tr_idx, te_idx) in enumerate(skf.split(X_df, y), start=1):
            X_tr_df, X_te_df = X_df.iloc[tr_idx], X_df.iloc[te_idx]
            y_tr, y_te = y[tr_idx], y[te_idx]
            X_tr = pre.fit_transform(X_tr_df)
            X_te = pre.transform(X_te_df)
            ...
            configs = []
            for v in sensitivity_grid["alpha"]:
                configs.append(("alpha", float(v), {"alpha": float(v)}))
            for v in sensitivity_grid["tau_majority"]:
                configs.append(("tau_majority", float(v), {"maj_ratio_thr": float(v)}))
            for v in sensitivity_grid["epsilon"]:
                configs.append(("epsilon", float(v), {"epsilon": float(v)}))
            for v in sensitivity_grid["mr_exponent"]:
                configs.append(("mr_exponent", float(v), {"mr_exponent": float(v)}))

            for factor, value, overrides in configs:
                common = dict(
                    k_neighbors=5, k_borderline=15, maj_ratio_thr=0.30,
                    alpha=6.0, epsilon=1e-12, mr_exponent=2.0,
                    max_majority_for_gate=2000, random_state=RANDOM_STATE,
                    use_gate=True, max_attempts_per_sample=12,
                    p_floor=0.15, p_ceil=0.98, fallback_accept=True,
                )
                common.update(overrides)
                if d_type == "numeric":
                    sampler = MoROGSampler(**common)
                else:
                    sampler = MoROMixSampler(
                        num_indices=num_indices, cat_indices=cat_indices, **common
                    )
                ...
                try:
                    ...
                    rows.append({
                        ...
                        "f1_macro": f1m,
                        "accuracy": acc,
                        "MeanMRStar": getattr(sampler.diag_, "mean_mr_star", np.nan),
                        "MeanAcceptProb": getattr(sampler.diag_, "mean_accept_prob", np.nan),
                        "RetryRate": getattr(sampler.diag_, "retry_rate", np.nan),
                        "FallbackAcceptRate": getattr(sampler.diag_, "fallback_accept_rate", np.nan),
                    })
                except Exception as e:
                    rows.append({
                        "dataset": ds_name, "openml_id": ds_id, "detected_type": d_type,
                        "factor": factor, "value": value, "fold": int(fold_idx),
                        "f1_macro": np.nan, "accuracy": np.nan,
                        "MeanMRStar": np.nan, "MeanAcceptProb": np.nan,
                        "RetryRate": np.nan, "FallbackAcceptRate": np.nan,
                        "error": repr(e),
                    })

    out = pd.DataFrame(rows)
    out_path = f"{output_prefix}_raw.csv"
